 ...
 */
#ifndef PACKET_NUMBER_INDEXED_QUEUE_H
#define PACKET_NUMBER_INDEXED_QUEUE_H
                                          ...
                                          Args &&... args)
{
    if (IsEmpty())
    {
        NS_ASSERT(entries_.empty());
        NS_ASSERT(0u == first_packet_);
    ...
    entries_.emplace_back(std::forward<Args>(args)...);
    NS_ASSERT(packet_number == last_packet());
    return true;
}
...
template <typename T>
auto PacketNumberIndexedQueue<T>::GetEntryWrapper(PacketNumber offset) const
    -> const EntryWrapper *
{
